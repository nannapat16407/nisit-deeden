...
const MOCK_DESCRIPTION =
  "นิสิตที่สนใจสมัครขอรับรางวัลนิสิตดีเด่น สามารถสมัครผ่านระบบออนไลน์ได้ตั้งแต่วันนี้เป็นต้นไป โดยต้องเตรียมเอกสารประกอบการสมัครให้ครบถ้วน";
const MOCK_TITLE = "Hello";
function Welcome() {
  return (
      ...
      <div className="">
        <div className="flex flex-col md:flex-row gap-4">
          <div className="w-full md:w-[80%] flex flex-col space-y-2 md:space-y-6">
            <p className="font-extrabold text-xl md:text-3xl text-black sm:whitespace-nowrap">
              ยินดีต้อนรับระบบนิสิตดีเด่น
            </p>
            <div className="flex flex-col sm:flex-row gap-2 sm:space-x-2">
              <p className="text-sm md:text-base text-black">
                สมัครขอรับรางวัล{MOCK_REWARD}ผ่านระบบออนไลน์
              </p>
              <div className="px-2 py-1 bg-emerald-600 rounded-xl flex items-center justify-center self-start">
                <p className="text-xs text-white whitespace-nowrap">
                  {MOCK_SEMESTER} {MOCK_ACADEMIC_YEAR}
                </p>
              </div>
            </div>
          </div>
          <div className="flex flex-col w-full md:w-[20%] justify-end items-stretch md:items-end">
            <button className="w-full py-2 bg-emerald-600 text-lg md:text-xl rounded-xl hover:cursor-pointer hover:bg-emerald-700 text-white">
              สมัคร
            </button>
          </div>
        </div>
...
function Countdown() {
  ...
  const start_date = currentPeriod?.start_date || "2026-01-01";
  const end_date = currentPeriod?.end_date || "2026-12-31";
  ...
  return (
    <div className="bg-white rounded-xl p-10 mt-4 flex flex-col justify-center items-center">
      <p className="text-black text-lg mb-4">
        ช่วงเวลาที่กำหนด ระหว่างวันที่ {formatDateToBE(start_date)} -{" "}
        {formatDateToBE(end_date)}
      </p>
  ...
  return (
    <div className="bg-white rounded-xl p-4 mt-4">
      ...
      <div className="flex flex-col">
        <div className="flex flex-col px-6 pb-6 gap-y-2">
          <p className="text-gray-700 font-bold">เรื่อง {MOCK_TITLE}</p>
